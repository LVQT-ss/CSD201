...
public class Graph {
    ...
    Graph() {
       //idx 0123456789012345
        v = "AEDCBFGHIJKLMNOP".toCharArray();
        deg = new int[20];
        a = new int[20][20];
        n = 0;
    }
    ...
    void f1() throws Exception {
        ...
        f.writeBytes("\r\n");
        /**
         * Question 3.1: Perform depth-first search on the graph.
         * Starting from the vertex D (which index is 2), 
         * ONLY display the vertex right after D up to the next 5 vertices (including D), and NOT display vertex 'C' (if any), but still consider it as one count during traversing the graph.
         * Hint: copy depth_rec(...) to depth_rec_2(...) and modify the method depth_rec_2(...) appropriately.
         ...
         *          D  A  C  G  E  B  H  I  F
         *          D  A  G  E
         * The output does not display vertex 'C', but still consider it as one count during traversing the graph.
         */
        //---------------------------------------------------------------------------------------
    ...
    void depth_rec_2(int k, int count, RandomAccessFile f) throws Exception {
        if (count > 5 || v[k] == 'C') { // Skip C and only consider up to 5 vertices
            return;
        }
        fvisit(k, f);
        for (int i = 0; i < n; i++) {
            if (a[k][i] > 0) {
                depth_rec_2(i, count + 1, f);
            }
        }
    }
